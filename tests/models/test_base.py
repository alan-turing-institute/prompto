...
from batch_llm.models import AsyncBaseModel, BaseModel
from batch_llm.settings import Settings


def test_base_model_init_errors(temporary_data_folders):
    ...
    with pytest.raises(TypeError, match="missing 1 required positional argument"):
        BaseModel(log_file="log_file")


def test_base_model_init(temporary_data_folders):
    # intialise settings object for BaseModel
    settings = Settings()

    # test that the base model class can be instantiated
    base_model = BaseModel(settings=settings, log_file="log_file")
    assert base_model.settings == settings
    assert base_model.log_file == "log_file"


def test_base_model_methods(temporary_data_folders):
    ...
    with pytest.raises(
        NotImplementedError,
        match="'check_environment_variables' method needs to be implemented by a subclass of BaseModel",
    ):
        BaseModel.check_environment_variables()
    # raise NotImplementedError as staticmethod call (with instance)
    with pytest.raises(
        NotImplementedError,
        match="'check_environment_variables' method needs to be implemented by a subclass of BaseModel",
    ):
        base_model.check_environment_variables()

    # check .check_prompt_dict()
    # raise NotImplementedError as staticmethod call (without instance)
    with pytest.raises(
        NotImplementedError,
        match="'check_prompt_dict' method needs to be implemented by a subclass of BaseModel",
    ):
        BaseModel.check_prompt_dict(prompt_dict={})
    # raise NotImplementedError as staticmethod call (with instance)
    with pytest.raises(
        NotImplementedError,
        match="'check_prompt_dict' method needs to be implemented by a subclass of BaseModel",
    ):
        base_model.check_prompt_dict(prompt_dict={})
    # raises TypeError if called without prompt_dict
    with pytest.raises(TypeError, match="missing 1 required positional argument"):
        base_model.check_prompt_dict()

    # check .query()
    # raises NotImplementedError as instance method
    with pytest.raises(
        NotImplementedError,
        match="'query' method needs to be implemented by a subclass of BaseModel",
    ):
        base_model.query(prompt_dict={})
    # raises TypeError if called without prompt_dict
    with pytest.raises(TypeError, match="missing 1 required positional argument"):
        base_model.query()
    ...
    # check .async_query()
    # should raise AttributeError as BaseModel does not have async_query method
    with pytest.raises(
        AttributeError, match="'BaseModel' object has no attribute 'async_query'"
    ):
        base_model.async_query(prompt_dict={})
